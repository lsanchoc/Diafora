...
function parseConvertedXml(convertedJson) {
    ...
    processedJson.number_of_results_returned =
        convertedJson.attributes.number_of_results_returned['_value'];
    processedJson.start = convertedJson.attributes.start['_value'];
    processedJson.total_number_of_results =
        convertedJson.attributes.total_number_of_results['_value'];
...
function loadParsedXmlResult(jsonXmlChild) {
    ...
    if (jsonXmlChild.accepted_name)
        jsonXmlChild = jsonXmlChild.accepted_name[0];
    ...
    if (
        jsonXmlChild.child_taxa !== undefined &&
        jsonXmlChild.child_taxa[0].taxon !== undefined
    ) {
        ...
        for (
            ...
            processedChild.name =
                childrenArray[childrenIndex].name[0]['_value'];
    ...
    newResult.synonyms = [];
    if (
        jsonXmlChild.synonyms !== undefined &&
        jsonXmlChild.synonyms[0].synonym !== undefined
    ) {
        ...
        for (
            ...
            processedSynonym.name =
                synonymsArray[synonymIndex].name[0]['_value'];
            processedSynonym.author =
                synonymsArray[synonymIndex].author[0]['_value'];
