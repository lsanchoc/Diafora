...
const binarySearch = (data, target, start, p_end) => {
  ...
  if (p_end - 1 === start) return (Math.abs(data[start].y - target.y) > Math.abs(data[p_end].y - target.y) ? p_end : start);
	...
	actual_lines.forEach(
		(line) => {
			...
			if(mom.c[target_index] == target){
				...
				//check if change produces an improvement is not exact becaus positions are not updated yet, but it will do
				if(getDistance(source,target,yPosL,yPosR) > getDistance(source,mom.c[newIndex],yPosL,yPosR)){
...
function updateP(options,p_lines,yPosL,yPosR){
	...
	p_lines.forEach(function(line){
		...
		line.t.p += direction* Math.abs(distance)* options.atractionForce/* line.a*/; //a es la cantidad de lineas representadas por ese nodo	
		line.o.p += -line.o.p;
